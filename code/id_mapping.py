...
def produce_pdbID_dict (inPath, spList, outPath):
    """Make a map of various UniProt IDs to PDB IDs.
    ...
    with open(inPath, 'r') as fIn:
        ...
        for line in fIn:
            ...
            if otherIDtype == 'PDB':
                if uniprotID in swissProtIDs:
                    if uniprotID in idMap:
                        idMap[uniprotID].add(otherID.lower())
                    else:
                        idMap[uniprotID] = {otherID.lower()}
    with open(outPath, 'wb') as fOut:
        pickle.dump(idMap, fOut)
